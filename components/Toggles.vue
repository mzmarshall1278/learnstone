<template>
  <div class="bg-beige-1 py-10 lg:py-24 lg:px-[132px] rounded-[32px]">
    <div class="lg:mt-20 w-[343px] lg:w-[1166px] mx-auto">
      <h5 class="text-orange font-semibold text-base">Platform</h5>
      <h2 class="font-semibold text-[40px] leading-[48px] mt-2 mb-14">
        Core Features
      </h2>
    </div>
    <div class="lg:flex gap-[132px] w-[343px] lg:w-[1166px] mx-auto">
      ...
      <div class="w-full lg:w-1/2">
        ...
        >
          ...
          <transition name="accordion" mode="out-in">
            ...
            >
              <p
                v-for="(para, index) in item.content"
                :key="index"
                class="font-normal mb-2 text-base"
              >
                {{ para }}
              </p>
              <img
                ...
                :key="activeItem"
                class="w-full h-full transition-opacity duration-500 ease-in-out object-contain lg:hidden"
                alt="Image"
              />
            </div>
          </transition>
      ...
      <div class="w-full lg:w-1/2 items-center justify-center hidden lg:flex">
        <transition name="fade" mode="out-in">
          <img
            v-if="activeItem !== null"
            :src="items[activeItem].image"
            :key="activeItem"
            class="w-full h-full transition-opacity duration-500 ease-in-out object-contain"
            alt="Image"
          />
        </transition>
      </div>
